...
import {
  Megaphone, AlertTriangle, ChevronDown, ChevronRight, Loader2,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { MESSAGE_TYPES } from "@/lib/communication/hooks";
import { useMyAnnouncements, useMarkStudentAnnouncementRead } from "@/lib/student-portal/hooks";
...
export function StudentAnnouncementsPanel({ schoolId, userId }: Props) {
  const { data: announcements = [], isLoading } = useMyAnnouncements(schoolId);
  const markRead = useMarkStudentAnnouncementRead(userId);
          ...
          return (
            <div
              ...
              className={cn(
                ...
                ann.is_emergency && "border-l-4 border-l-destructive bg-destructive/5",
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {(ann as AnnouncementWithSender).profiles?.full_name ?? "School"} ·{" "}
