...
import { useChildAttendance, useChildAttendanceSummary } from "@/lib/parent/hooks";
...
export function ChildAttendanceCard({ studentId, compact = false }: Props) {
  const { data: records = [], isLoading } = useChildAttendance(studentId);
  const summary = useChildAttendanceSummary(studentId);
  ...
  const pctColor = summary.pct >= 90 ? "text-success" : summary.pct >= 75 ? "text-amber-500" : "text-destructive";
  const progressColor = summary.pct >= 90 ? "bg-success" : summary.pct >= 75 ? "bg-amber-500" : "bg-destructive";
  ...
  // Last 30 records for the calendar grid
        ...
        {streak >= 3 && (
          <div className="flex items-center gap-2 rounded-lg bg-amber-500/10 border border-amber-500/20 px-3 py-2">
            ...
            <p className="text-xs font-medium text-amber-600 dark:text-amber-400">
              {streak} day streak of attendance!
            </p>
          </div>
        )}
            ...
            <div className="flex flex-wrap gap-1">
              {recent.map((r) => (
                <div
                  ...
                  className={cn(
                    "h-4 w-4 rounded-sm",
                    STATUS_COLOR[r.status] ?? "bg-muted",
                  )}
                />
              ))}
            </div>
